...
When the viscous term is included, @eq-burgers is parabolic. The inviscid Burgers' equation arises when viscous effects are zero, $\mu=0$.

$$
\frac {\partial u}{\partial t}+u\frac {\partial u}{\partial x}=0
$$ {#eq-inviscid-burgers}

Whereas the linear 1D convection equation models the simple case of a wave propagating at a constant speed, @eq-inviscid-burgers may be viewed as a nonlinear wave equation that can model waves propagating at varying speeds. Since Burgers' equation can correctly model varying wave speeds, it can be used to model shocks as waves of differing speeds coalesce. This wave equation is similarly hyperbolic.
...
Expand $u_j^{n+1}$ about $u_j^n$ using a Taylor series and substitute the time derivative term with the spatial derivative. From @eq-general-hyperbolic-pde, we have that $u_t=-F_x$. Therefore
...
where the dimensionless wave number $\beta$ is $k\Delta x$. For Burgers' equation, $F=u^2/2$, so the Jacobian is simply $A(u)=u$. Stability occurs when $|G|\leq1$. For the $G$ above, @eq-lax-method-inviscid-burgers-amplification-factor reduces down to
...
The Lax-Wendroff method was one of the first second order finite difference methods derived for hyperbolic PDEs. Once again, consider the general form of the governing @eq-general-hyperbolic-pde. The Lax-Wendroff method is
...
Next, replace the time derivatives with spatial derivatives. From @eq-general-hyperbolic-pde, the first time derivative is
...
Consider the control volume shown below. The $x$ and $y$ axes are space and time respectively. The control volume extends from $x_{j-\frac 12}$ to $x_{j+\frac 12}$ in space and from $t^n$ to $t^{n+1}$ in time. The center of the control volume is at $\left(j,n+\frac 12\right)$.

![Control volume for the Godunov scheme](../assets/chapter-4/lax-control-volume.png){width=550 .lightbox}

Integrating @eq-general-hyperbolic-pde across the control volume, then the volume-averaged $u$ and time-averaged flux $f$ are
...
Integrating @eq-general-hyperbolic-pde and swapping the integration order, then
...
Solving for $\overline u_j^{n+1}$ completes the proof for @eq-godunov-scheme-inviscid-burgers-conservation-equation.
...
![Initial wave diagram for the Godunov scheme](../assets/chapter-4/godunov-riemann-problem-initial-wave-diagram.png){width=550 .lightbox}
...
then the solution to the Riemann problem for @eq-general-hyperbolic-pde is: for shocks ($u_j>u_{j+1}$)
...
0 \qquad\quad & \qquad\quad & u_j<0<u_{j+1} \\ \\
...
@eq-inviscid-burgers becomes, after normalizing by $a$,

$$
\frac {\partial u}{\partial t'}+u\frac {\partial u}{\partial x'}=0
$$

Therefore, @eq-inviscid-burgers has no inherent time or length scale. If $u(x,t)$ is a solution to @eq-inviscid-burgers, then $u(ax,at)$ must also be a solution to the PDE in the scaled coordinate system. Invariance can also be shown for the initial condition by inspecting $u(ax,0)$. Taking $u(ax,0)$ at the $j+\frac 12$ cell boundary, then
...
Since $a>0$, the $ax<0$ and $ax>0$ conditions are identical to $x<0$ and $x>0$. Therefore, $u(ax,0)=u(x,0)$ and the initial condition is also invariant to uniform scaling. Because both the PDE and initial condition are unchanged by scaling, then the solutions $u(ax,at)$ and $u(x,t)$ must be the same for all $a>0$. Let $a=\frac 1t$ to reveal that $u(ax,at)=u(\xi,1)$, where $\xi=\frac xt$ is the self-similarity parameter.
